...
ask_system() {
  ...
  echo
  info "Ejemplos de timezone: America/Argentina/Buenos_Aires  America/Mexico_City"
...
update_mirrors() {
  ...
  if ! ping -c 1 -W 3 archlinux.org &>/dev/null &&
     ! curl -s --connect-timeout 3 https://archlinux.org >/dev/null 2>&1; then
    warn "Sin conectividad — saltando reflector, usando mirrors actuales"
    return 0
  fi
...
if efibootmgr --create \
  ...
  echo "[chroot]    efibootmgr --create --disk /dev/$DISK --part $PART_NUM --label 'Helix Linux' --loader '\\EFI\\BOOT\\BOOTX64.EFI'"
fi
...
if systemctl list-unit-files limine-snapper-watcher.service &>/dev/null; then
  ...
  echo "[chroot]       paru -S limine-snapper-sync limine-mkinitcpio-hook"
fi
...
echo "[chroot] Instalando paru (AUR helper)..."
# Primero intentar desde repo (core_repo puede tenerlo), fallback a AUR
if ! pacman -S --noconfirm paru 2>/dev/null; then
  echo "[chroot] Compilando paru desde AUR..."
  runuser -u "${USERNAME}" -- bash -c '
    cd /tmp
    git clone --depth=1 https://aur.archlinux.org/paru.git 2>/dev/null || { echo "WARN: git clone falló"; exit 1; }
    cd paru
    makepkg -si --noconfirm
  ' 2>&1 || echo "[chroot] WARN: No se pudo instalar paru — instalalo manualmente después"
fi
